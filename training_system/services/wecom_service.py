...
def send_new_student_notification(student_name, training_type, id_card, phone):
    ...
    # 企业微信 textcard 支持特定协议拉起小程序:
    # 格式: http://servicewechat.com/{appid}/{version}/page-path?{query}
    # {version} 可以是 0 (正式版), 1 (开发版), 2 (体验版)
    if mini_appid:
        target_url = f"http://servicewechat.com/{mini_appid}/0/page-frame.html"
    else:
