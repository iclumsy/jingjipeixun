...
1. `generate_student_materials`
2. `process_diploma` / `process_id_cards` / `process_hukou`
3. `auto_crop_diploma` / `auto_crop_id_card` / `auto_crop_hukou_page`
4. `auto_crop_with_profile`
5. `detect_document_candidates` + `select_best_candidate` + `crop_with_fallback`

核心思路:

1. 不再使用“找到第一个四边形就直接透视”的单一路径。
2. 改为按材料类型分流:
   - `id_card`: 身份证，使用固定宽高比强约束
   - `hukou`: 户口页，检测前先抑制内部表格线
   - `diploma`: 毕业证书，按页面类材料评分
3. 每类材料都会先做亮度/对比度评估，在低光照下按需启用温和增强。
4. 轮廓检测后保留多个候选框，按面积占比、矩形度、边界连续性、边缘距离和材料类型特征综合评分。
5. 仅当候选分数达到门槛时才做透视校正；否则回退到保守矩形裁剪，必要时保留原图。

身份证额外约束:

1. 标准比例约为 `1.585`。
2. 候选先过宽高比粗筛，再参与综合评分。
3. 透视结果会再次校验宽高比，避免把错误候选拉歪后继续输出。

户口页额外约束:

1. 使用形态学方法提取并减弱细长水平/垂直表格线。
2. `inner_line_density` 参与评分，避免表格网格反客为主。

低光照与导出增强:

1. `analysis` 阶段增强优先服务边缘检测。
2. `export` 阶段增强优先服务最终可读性。
3. 若增强后出现明显过曝或改善不足，则回退到未增强版本。

人工回归建议:

1. 用复杂背景身份证样本检查是否还会把桌面边框带入。
2. 用表格线明显的户口页样本检查是否仍会误裁内部网格。
3. 用低光照毕业证书样本检查导出图是否更清楚且不过曝。
