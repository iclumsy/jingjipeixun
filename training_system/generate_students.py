...
用于开发和测试阶段快速填充数据库，模拟真实学员数据。

功能:
    1. 随机生成学员基本信息（姓名、性别、身份证号、手机号等）
    2. 随机选取单位名称和地址
    3. 从作业类别配置文件中随机选择作业类别和操作项目
    4. 生成占位附件文件路径（或从在线 API 下载真实人像照片）
    5. 批量插入数据库

用法:
    python generate_students.py [--count N] [--type special_operation|special_equipment]

注意:
    - 生成的身份证号为随机构造，不符合真实校验规则
    - 附件文件使用占位图片，生产环境不应使用此脚本
"""
import os
import random
import sqlite3
